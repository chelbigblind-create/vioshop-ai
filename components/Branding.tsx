...
const Branding: React.FC = () => {
  ...
  const generateLogo = async () => {
    ...
    try {
      ...
      setError("Erro ao gerar logo. Verifique sua chave.");
  ...
  const generateThumbnail = async () => {
    ...
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const prompt = "A vibrant, eye-catching TikTok video thumbnail cover. High contrast, featuring a futuristic 3D product (like a glowing blender or headset) with floating text elements like 'TOP 1' or '70% OFF' in bold cinematic typography. Pink and purple neon lighting, professional studio product photography style, vertical 9:16 aspect ratio.";
    ...
    <div className="space-y-12 animate-in fade-in duration-700 pb-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-10 rounded-[3rem] shadow-2xl space-y-8">
          ...
          <div className="aspect-square bg-zinc-950 rounded-[2.5rem] border border-zinc-800 flex items-center justify-center overflow-hidden relative group">
            {logoUrl ? <img src={logoUrl} className="w-full h-full object-cover" /> : <i className="fas fa-image text-4xl text-zinc-800"></i>}
        ...
        {/* GERADOR DE CAPAS (NOVO) */}
        <div className="bg-zinc-900 border border-zinc-800 p-10 rounded-[3rem] shadow-2xl space-y-8">
          <div className="space-y-4">
             <div className="w-12 h-12 bg-indigo-500/10 rounded-2xl flex items-center justify-center text-indigo-500">
                <i className="fas fa-rectangle-ad text-xl"></i>
             </div>
             <h3 className="text-2xl font-black text-white italic uppercase">Capa de Vídeo (Thumbnail)</h3>
             <p className="text-zinc-500 text-xs leading-relaxed uppercase font-bold tracking-widest">Estude estilos de capas que param o scroll no TikTok.</p>
          </div>

          <div className="aspect-[9/16] h-[300px] mx-auto bg-zinc-950 rounded-[2rem] border border-zinc-800 flex items-center justify-center overflow-hidden relative group">
            {thumbUrl ? <img src={thumbUrl} className="w-full h-full object-cover" /> : <i className="fas fa-clapperboard text-4xl text-zinc-800"></i>}
